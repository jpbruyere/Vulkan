...
public:
    ...
    VulkanExample() : VulkanExampleBase(ENABLE_VALIDATION)
    {
        ...
        zoom = -50.5f;
        rotation = { 190.2f, 4.7f, 0.0f };
        cameraPos = { 5.5f, 10.85f, 0.0f };
        rotationSpeed = 0.25f;
    ...
    void prepareInstanceData()
    {
        ...
        /*for (auto i = 0; i < INSTANCE_COUNT / 2; i++) {
            ...
            instanceBuffer.instanceData[i].pos = glm::vec3(rho*cos(theta), uniformDist(rndGenerator) * 0.5f - 0.25f, rho*sin(theta));
            instanceBuffer.instanceData[i].rot = glm::vec3(M_PI * uniformDist(rndGenerator), M_PI * uniformDist(rndGenerator), M_PI * uniformDist(rndGenerator));
            instanceBuffer.instanceData[i].scale = 1.5f + uniformDist(rndGenerator) - uniformDist(rndGenerator);
            instanceBuffer.instanceData[i].texIndex = rndTextureIndex(rndGenerator);
            instanceBuffer.instanceData[i].scale *= 10.75f;

            // Outer ring
            rho = sqrt((pow(ring1[1], 2.0f) - pow(ring1[0], 2.0f)) * uniformDist(rndGenerator) + pow(ring1[0], 2.0f));
            theta = 2.0 * M_PI * uniformDist(rndGenerator);
            instanceBuffer.instanceData[i + INSTANCE_COUNT / 2].pos = glm::vec3(rho*cos(theta), uniformDist(rndGenerator) * 0.5f - 0.25f, rho*sin(theta));
            instanceBuffer.instanceData[i + INSTANCE_COUNT / 2].rot = glm::vec3(M_PI * uniformDist(rndGenerator), M_PI * uniformDist(rndGenerator), M_PI * uniformDist(rndGenerator));
            instanceBuffer.instanceData[i + INSTANCE_COUNT / 2].scale = 1.5f + uniformDist(rndGenerator) - uniformDist(rndGenerator);
            instanceBuffer.instanceData[i + INSTANCE_COUNT / 2].texIndex = rndTextureIndex(rndGenerator);
            instanceBuffer.instanceData[i + INSTANCE_COUNT / 2].scale *= 0.75f;
        }*/
    ...
    void init_physics() {
        ...
        groundRigidBodyCI.m_restitution = 0.98;
        ...
        fallRigidBodyCI.m_friction = 1.5;
    ...
    void update_physics () {
        ...
        for (auto i = 0; i < INSTANCE_COUNT; i++) {
            data[i].pos = glm::vec3(p.getX(),p.getY(),p.getZ());
        }
